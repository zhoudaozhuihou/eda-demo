...
export function ApiDocContent({ api }: { api: API }) {
  ...
  useEffect(() => {
    const onPopState = (e: PopStateEvent) => {
      if (e.state && e.state.section) {
        scrollTo(e.state.section, false);
      }
    };
  ...
  return (
    <div className="flex flex-col md:flex-row h-full relative items-start overflow-hidden">
      {/* Mobile Nav Toggle */}
      <div className="md:hidden w-full flex items-center justify-between mb-4 sticky top-0 bg-background z-20 py-2 border-b px-4 flex-none">
        <span className="font-semibold">{t('doc.navigation', { defaultValue: 'Navigation' })}</span>
      ...
      {/* Mobile Nav Dropdown */}
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-10 left-0 right-0 bg-background border rounded-lg shadow-lg z-30 p-2 m-2">
          {sections.map((s) => (
            <button
              key={s.id}
              onClick={() => scrollTo(s.id)}
              className={`block w-full text-left px-3 py-2 text-sm rounded ${
                activeSection === s.id ? 'bg-primary/10 text-primary font-medium' : 'hover:bg-accent'
              }`}
            >
              {s.label}
            </button>
        ...
        <div
          ...
          onClick={(e) => {
            ...
            if (target && target.dataset.sectionId) {
              scrollTo(target.dataset.sectionId);
            }
          }}
        >
          {sections.map((s) => (
      ...
      <div id="doc-content-area" className="flex-1 min-w-0 h-full overflow-y-auto p-4 sm:p-6 space-y-10 pb-10">
        <section id="doc-section-overview" className="scroll-mt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm mb-6">
            ...
            <div className="min-w-0 flex items-center">
              ...
              <span className="ml-2 truncate" title={api.authType}>
                {api.authType}
              </span>
            </div>
            <div className="min-w-0 flex items-center">
              <span className="text-muted-foreground flex-shrink-0">{t('doc.labels.domain')}</span>
              <span className="ml-2 truncate" title={api.domain}>
                {api.domain}
              </span>
            </div>
          </div>
        ...
        <section id="doc-section-response" className="scroll-mt-4">
          <h3 className="mb-4 text-lg font-semibold">{t('doc.tabs.response')}</h3>
          <ApiDocResponseTree params={doc.response} />
        </section>

        <div className="w-full h-px bg-border" />

        <section id="doc-section-status" className="scroll-mt-4">
          <h3 className="mb-4 text-lg font-semibold">{t('doc.tabs.status')}</h3>
